    ...
    for kj in k_plot:
        for ki in k_par:
            ...
            if (ki >= (Ncav - 1) * dk / 2) or (ki <= - (Ncav - 1) * dk / 2):
                # DS contribution
                tmp1[count] += np.real(np.sum(cb**2 * (1 + Bose(temp[Ti], wb)) / (Nexc * (omega_LP(wc, kj) - w0 - wb + 1.0j * gammak))
                                  + cb**2 * Bose(temp[Ti], wb) / (Nexc * (omega_LP(wc, kj) - w0 + wb + 1.0j * gammak))))
...
#                                  + cb**2 * Bose(wb) / (Nexc * (omega_UP(wc, kj) - w0 + wb + 1.0j * gammak))))
            ...
            if (- (Ncav - 1) * dk / 2 < ki < (Ncav - 1) * dk / 2):
                ...
                # LP contributions
                tmp1[count] += np.real(np.sum(sin_2_theta * wb * cb**2 * (1 + Bose(temp[Ti], wb)) / (Nexc * (omega_LP(wc, kj) - omega_LP(wc, ki) - wb + 1.0j * gammak))
                                  + sin_2_theta * wb * cb**2 * Bose(temp[Ti], wb) / (Nexc * (omega_LP(wc, kj) - omega_LP(wc, ki) + wb + 1.0j * gammak))))
...
#                                  + sin_2_theta * wb * cb**2 * Bose(wb) / (Nexc * (omega_UP(wc, kj) - omega_LP(wc, ki) + wb + 1.0j * gammak))))
                ...
                # UP contribution
                tmp1[count] += np.real(np.sum(cos_2_theta * wb * cb**2 * (1 + Bose(temp[Ti], wb)) / (Nexc * (omega_LP(wc, kj) - omega_UP(wc, ki) - wb + 1.0j * gammak))
                                  + sin_2_theta * wb * cb**2 * Bose(temp[Ti], wb) / (Nexc * (omega_LP(wc, kj) - omega_UP(wc, ki) + wb + 1.0j * gammak))))
