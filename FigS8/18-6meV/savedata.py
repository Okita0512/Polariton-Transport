...
gamma = 0.0186 / conv             # characteristic frequency
ndof = 40000                      # number of bath DOFs
...
for kj in k_plot:
        ...
        tmp2[count] += np.real(np.sum(cb**2 * (1 + Bose(wb)) / ((omega_UP(wc, kj) - w0 - wb + 1.0j * gammak))
                                  + cb**2 * Bose(wb) / ((omega_UP(wc, kj) - w0 + wb + 1.0j * gammak))))
...
np.savetxt("band_struct.txt", band_struct)
...
def vk_tilde(tilde_wk, k):
    ...
    for i in range(1, len(k) - 1):
        vel[i - 1] = (tilde_wk[i + 1] - tilde_wk[i - 1]) / (2 * (k[1] - k[0]))
