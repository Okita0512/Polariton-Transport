...
k_plot = np.array([k2 - window, k2 + window]) 
...
tilde_wk0 = np.zeros((len(temp), len(k_plot)), dtype = float)
tilde_wk1 = np.zeros((len(temp), len(k_plot)), dtype = float)
tilde_wk2 = np.zeros((len(temp), len(k_plot)), dtype = float)
vel = np.zeros((len(temp), 3), dtype = float)

for Ti in range(len(temp)):
    
    tmp1 = np.zeros((len(k_plot)), dtype = float)   # relaxation term
    tmp2 = np.zeros((len(k_plot)), dtype = float)   # excitation term

    count = 0
    for kj in k_plot:

        # DS contribution
        tmp1[count] += np.real(np.sum(cb**2 * (1 + Bose(temp[Ti], wb)) / (Nexc * (omega_LP(wc, kj) - w0 - wb + 1.0j * gammak)))) * (Nexc - Ncav)
            
        tmp2[count] += np.real(np.sum(cb**2 * Bose(temp[Ti], wb) / (Nexc * (omega_LP(wc, kj) - w0 + wb + 1.0j * gammak)))) * (Nexc - Ncav)
    ...
    tilde_wk0[Ti, :] = omega_LP(wc, k_plot) + 2 * (tmp1 + tmp2) * sin_2_thetak
    tilde_wk1[Ti, :] = omega_LP(wc, k_plot) + 2 * tmp1 * sin_2_thetak
    tilde_wk2[Ti, :] = omega_LP(wc, k_plot) + 2 * tmp2 * sin_2_thetak

    vel[Ti, 0] = (tilde_wk0[Ti, 1] - tilde_wk0[Ti, 0]) / (k_plot[1] - k_plot[0])
    vel[Ti, 1] = (tilde_wk1[Ti, 1] - tilde_wk1[Ti, 0]) / (k_plot[1] - k_plot[0])
    vel[Ti, 2] = (tilde_wk2[Ti, 1] - tilde_wk2[Ti, 0]) / (k_plot[1] - k_plot[0])
