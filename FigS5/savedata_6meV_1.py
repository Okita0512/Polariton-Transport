...
# case 1
lam = 0.006 / conv                 # reorganization energy
...
for kj in k_plot:
    ...
    tmp2[count] += np.real(np.sum(cb**2 * (1 + Bose(wb)) / (Nexc * (omega_UP(wc, kj) - w0 - wb + 1.0j * gammak))
                    + cb**2 * Bose(wb) / (Nexc * (omega_UP(wc, kj) - w0 + wb + 1.0j * gammak)))) * (Nexc - Ncav)
...
np.savetxt("band_struct_LP_lambda=6meV_1.txt", band_struct)
...
def vk_tilde(tilde_wk, k):
    ...
    for i in range(1, len(k) - 1):
        vel[i] = (tilde_wk[i + 1] - tilde_wk[i - 1]) / (2 * (k[1] - k[0]))

    return vel[1:-1]

group_vel_LP = np.zeros((Ncav - 2, 2), dtype = float)
group_vel_LP[:, 0] = omega_LP(wc, k_plot)[1:-1]
group_vel_LP[:, 1] = vk_tilde(tilde_wk1, k_plot)

group_vel_UP = np.zeros((Ncav - 2, 2), dtype = float)
group_vel_UP[:, 0] = omega_UP(wc, k_plot)[1:-1]
group_vel_UP[:, 1] = vk_tilde(tilde_wk2, k_plot)

np.savetxt("group_vel_LP_lambda=6meV_1.txt", group_vel_LP)
np.savetxt("group_vel_UP_lambda=6meV_1.txt", group_vel_UP)
